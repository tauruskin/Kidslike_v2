...
const getAllChildren = () => async dispatch => {
  ...
  try {
    const response = await axios.get(`${domain}/api/childs/`);
...
const addChildren = children => async dispatch => {
  ...
  try {
    ...
    const response = await axios.post(`${domain}/api/childs/`, { ...children });
...
const updateChildren = (data, id) => async dispatch => {
  ...
  try {
    await axios.patch(`${domain}/api/childs/${id}`, data).then(res => {
